...
const Food = () => {
    ...
    const [totals, setTotals] = useState({ totalPrice: 0, totalQuantity: 0, totalCalories: 0 });
  ...
  kismis: {
        ...
        return (
            <div className="absolute top-full mt-2 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-xl shadow-2xl p-5 border border-emerald-500/20 backdrop-blur-xl min-w-80 z-50">
                ...
                <div className="grid grid-cols-7 gap-1 mb-3">
                    {dayNames.map(day => (
                        <div key={day} className="text-center text-xs text-emerald-300/70 font-semibold py-2">
                            {day}
                        </div>
                    ))}
                </div>
                ...
                <div className="flex gap-2 text-xs">
                    <button onClick={() => handleDaySelect(new Date().getDate())} className="flex-1 py-2 bg-slate-700/40 hover:bg-slate-600/60 text-emerald-300 rounded-lg transition-colors">
                        Today
                    </button>
                    <button onClick={onClose} className="flex-1 py-2 bg-slate-700/40 hover:bg-slate-600/60 text-emerald-300 rounded-lg transition-colors">
                        Close
                    </button>
                </div>
            </div>
    ...
    const fetchFoods = async () => {
        ...
        try {
            ...
            const res = await axios.get('https://personal-tracker-x4kn.onrender.comapi/food', {
                headers,
                params
            });
            ...
            // Calculate totals
            const totalPrice = foodData.reduce((sum, food) => sum + (Number(food.price) || 0), 0);
            const totalQuantity = foodData.reduce((sum, food) => sum + (Number(food.quantity) || 0), 0);
            const totalCalories = foodData.reduce((sum, food) => sum + (Number(food.calories) || 0), 0);
            ...
            setError(err.response?.data?.message || err.message || 'Failed to load food items');
            setFoods([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editId
                ? `https://personal-tracker-x4kn.onrender.comapi/food/${editId}`
                : 'https://personal-tracker-x4kn.onrender.comapi/food';
            ...
            const errorMsg = err.response?.data?.message || err.message || 'Failed to save food';
            setError(errorMsg);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this item?')) {
            try {
                ...
                await axios.delete(`https://personal-tracker-x4kn.onrender.comapi/food/${id}`, {
                    headers
                });
                fetchFoods();
            } catch (err) {
                console.error('Error deleting food:', err);
                const errorMsg = err.response?.data?.message || err.message || 'Failed to delete food';
                setError(errorMsg);
            }
        }
    };
    ...
    const handleExport = async () => {
        try {
            ...
            const res = await axios.get('https://personal-tracker-x4kn.onrender.comapi/food/export', {
                headers,
                params,
                responseType: 'blob'
            });
            ...
            const filename = contentDisp?.split('filename=')?.[1] || 'food-export.txt';
            ...
            <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-emerald-400 via-cyan-400 to-blue-500 bg-clip-text text-transparent">Food Tracker</h1>
                            </div>
                            <p className="text-slate-400 text-sm sm:text-base">Manage and track your food inventory with style</p>
                        </div>
                        <div className="hidden lg:block text-right">
                            <div className="text-sm text-slate-400 mb-2">Totals</div>
                            <div className="space-y-1 text-sm">
                                <div><span className="text-emerald-400 font-semibold">₹{totals.totalPrice}</span></div>
                                <div><span className="text-cyan-400 font-semibold">{totals.totalQuantity} qty</span></div>
                                <div><span className="text-yellow-400 font-semibold">{totals.totalCalories} kcal</span></div>
                            </div>
                        </div>
                    </div>
                </header>
                ...
                <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-4 sm:p-6 mb-6 border border-slate-700/50 shadow-2xl">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        <div className="relative">
                            <label className="text-xs sm:text-sm font-semibold text-transparent bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text block mb-2">Start Date</label>
                            <input
                                type="text"
                        ...
                        <div className="relative">
                            <label className="text-xs sm:text-sm font-semibold text-transparent bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text block mb-2">End Date</label>
                            <input
                                type="text"
                                placeholder="YYYY-MM-DD"
                                value={endDate}
                        ...
                        <div>
                            <label className="text-xs sm:text-sm font-semibold text-transparent bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text block mb-2">Category Filter</label>
                            <select
                        ...
                        <div>
                            <label className="text-xs sm:text-sm font-semibold text-transparent bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text block mb-2">Actions</label>
                            <button
                                onClick={() => setShowForm(true)}
                                className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-600 hover:to-cyan-600 text-white px-4 py-2.5 rounded-xl transition transform hover:scale-105 active:scale-95 font-semibold shadow-lg hover:shadow-emerald-500/50"
                            >
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <button
                            ...
                            className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white px-4 sm:px-6 py-2.5 rounded-xl transition transform hover:scale-105 active:scale-95 font-semibold shadow-lg hover:shadow-blue-500/50"
                        >
                            <Download size={20} /> Export Data
                        </button>
                        <button
                            onClick={fetchFoods}
                            className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-600 hover:to-pink-700 text-white px-4 sm:px-6 py-2.5 rounded-xl transition transform hover:scale-105 active:scale-95 font-semibold shadow-lg hover:shadow-purple-500/50"
                        >
                            🔄 Refresh
                        </button>
                    </div>
                ...
                {error && (
                    <div className="mb-6 p-4 bg-rose-500/10 border border-rose-500/30 rounded-lg text-rose-300 text-sm flex items-center justify-between">
                        <span>{error}</span>
                        <button onClick={() => setError('')} className="text-rose-300 hover:text-rose-200">
                            <X size={18} />
                        </button>
                    </div>
                )}
                        ...
                        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                            ...
                            <div className="sm:col-span-1 bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 px-4 py-3 rounded-xl border border-emerald-500/30 backdrop-blur-sm">
                                <p className="text-slate-300 text-sm font-semibold">
                                    🔥 Calories: <span className="text-transparent bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text">{getCaloriesFromNutrition(formData.foodName)}</span> <span className="text-emerald-400">kcal</span>
                                </p>
                            </div>
                            <textarea
                ...
                <section>
                    ...
                    <div className="bg-slate-800/30 rounded-xl shadow-xl overflow-auto border border-emerald-500/10 backdrop-blur-sm">
                        <table className="min-w-full table-auto divide-y divide-slate-700/50">
                            <thead className="bg-gradient-to-r from-slate-900/80 to-slate-800/80">
                                <tr>
                                    <th className="px-4 py-3 text-left text-sm text-emerald-300 font-semibold">Date</th>
                                    <th className="px-4 py-3 text-left text-sm text-emerald-300 font-semibold">Food Name</th>
                                    <th className="px-4 py-3 text-left text-sm text-emerald-300 font-semibold">Category</th>
                                    <th className="px-4 py-3 text-right text-sm text-emerald-300 font-semibold">Price</th>
                                    <th className="px-4 py-3 text-center text-sm text-emerald-300 font-semibold">Quantity</th>
                                    <th className="px-4 py-3 text-center text-sm text-emerald-300 font-semibold">Calories</th>
                                    <th className="px-4 py-3 text-left text-sm text-emerald-300 font-semibold">Notes</th>
                                    <th className="px-4 py-3 text-right text-sm text-emerald-300 font-semibold">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-700/50">
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan="8" className="p-12">
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <p className="text-slate-400 text-lg mb-6">No food items found</p>
                                                <button
                                                    onClick={() => setShowForm(true)}
                                                    className="px-6 py-2 bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-600 hover:to-cyan-600 text-white rounded-xl font-semibold transition"
                                                >
                                                    Add your first item
                                                </button>
                                            </div>
                                    ...
                                    foods.map((food) => (
                                        <tr key={food._id} className="hover:bg-emerald-500/5 transition-colors duration-150">
                                            <td className="px-4 py-3 text-sm text-gray-200 whitespace-nowrap">{(food.purchaseDate || "").slice(0, 10)}</td>
                                            <td className="px-4 py-3 text-sm text-gray-100 font-medium">{food.foodName}</td>
                                            <td className="px-4 py-3 text-sm">
                                                <span className="px-3 py-1 rounded-full text-xs font-medium bg-emerald-500/20 text-emerald-300">
                                                    {food.category}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3 text-sm text-right font-semibold text-emerald-400">₹{food.price}</td>
                                            <td className="px-4 py-3 text-sm text-center text-cyan-400 font-semibold">{food.quantity} {food.unit}</td>
                                            <td className="px-4 py-3 text-sm text-center text-yellow-400 font-semibold">{food.calories || 0} kcal</td>
                                            <td className="px-4 py-3 text-sm text-gray-400 max-w-xs truncate">{food.notes || '-'}</td>
                                            <td className="px-4 py-3 text-sm text-right">
                                                <div className="inline-flex gap-2">
                                                    <button
                                                        onClick={() => handleEdit(food)}
                                                        className="px-3 py-1 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 rounded-lg text-xs font-medium transition-all duration-200 shadow-md hover:shadow-blue-500/30 text-white"
                                                    >
                                                        Edit
                                                    </button>
                                                    <button
                                                        onClick={() => handleDelete(food._id)}
                                                        className="px-3 py-1 bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-500 hover:to-red-500 rounded-lg text-xs font-medium transition-all duration-200 shadow-md hover:shadow-rose-500/30 text-white"
                                                    >
                                                        Delete
                                                    </button>
                                                </div>
